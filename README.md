<div align="center">
  <img src="https://raw.githubusercontent.com/mudler/LocalAI/master/core/http/static/logo.png" alt="LocalAI Logo" width="150" />
...
## <a id="configuration--usage"></a>⚙️ Configuration & Usage
...
If you are building your own NixOS configuration or another Flake-based project, you can easily add this LocalAI package as an input:

```nix
{
  inputs.localai.url = "github:Azteczek/LocalAI-On-NixOS";

  outputs = { self, nixpkgs, localai }: {
    # You can now use localai.packages.${system}.default in your configuration!
  };
}
```
...
### Manual Setup (If you cloned the repo)
...
./run.sh
```
*(By default, the script binds LocalAI to `127.0.0.1:8080` and looks for models in `/home/user/localai/models`. **Make sure to open `run.sh` in an editor and change the path to match your username/setup!**)*

---

## <a id="desktop-integration"></a>🖥 Desktop Integration
...
**Fix**: Edit `run.sh` and change the hardcoded path (`/home/user/localai/models`) to exactly match where your downloaded models actually live (e.g., `/home/yourusername/localai/models`).
...
This project is open-source and available under the MIT License - feel free to build upon it!
