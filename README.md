...
./run.sh
```
*(By default, the script binds LocalAI to `127.0.0.1:8080` and looks for models in `/home/krystian/localai/models`. **Make sure to open `run.sh` in an editor and change the path to match your username/setup!**)*
...
**Fix**: Edit `run.sh` and change the hardcoded path (`/home/krystian/localai/models`) to exactly match where your downloaded models actually live (e.g., `/home/yourusername/localai/models`).
